...
# RDS/Aurora Reserved Instance 一括購入スクリプト v3.2 (Security Reviewed)
...
validate_line() {
  ...
  case "$engine" in
    aurora-mysql|aurora-postgresql|mysql|postgresql|mariadb|oracle-se2|sqlserver-ex) ;;
...
echo -e "${CYAN}RDS/Aurora RI 購入スクリプト v3.2${NC}"
...
while IFS=',' read -r REGION DB_TYPE DB_INSTANCE_CLASS ENGINE MULTI_AZ_INPUT QUANTITY DURATION_YEARS PAYMENT_OPTION || [ -n "${REGION:-}" ]; do
  ...
  OFFERINGS=$(aws rds describe-reserved-db-instances-offerings \
    ...
    --query 'ReservedDBInstancesOfferings[0].[ReservedDBInstancesOfferingId,FixedPrice,UsagePrice,CurrencyCode]' \
  ...
  USAGE_PRICE=$(echo "$OFFERINGS" | jq -r '.[2]')
  CURRENCY=$(echo "$OFFERINGS" | jq -r '.[3]')

  echo -e "  ${GREEN}✓ Offering: $OFFERING_ID (初期: $FIXED_PRICE $CURRENCY, 時間: $USAGE_PRICE $CURRENCY)${NC}"

  if [ "$DB_TYPE" = "Aurora" ]; then
    RUNNING_COUNT=$(aws rds describe-db-instances --region "${REGION}" \
      --query "DBInstances[?DBInstanceClass==\`${DB_INSTANCE_CLASS}\` && starts_with(Engine, \`${ENGINE}\`) && DBInstanceStatus==\`available\`] | length(@)" \
      --output text 2>&1) || {
    ...
    RUNNING_COUNT=$(aws rds describe-db-instances --region "${REGION}" \
      --query "DBInstances[?DBInstanceClass==\`${DB_INSTANCE_CLASS}\` && Engine==\`${ENGINE}\` && MultiAZ==\`${MULTI_AZ_BOOL}\` && DBInstanceStatus==\`available\`] | length(@)" \
      --output text 2>&1) || {
      echo -e "  ${YELLOW}⚠ 稼働インスタンス数の取得に失敗しました${NC}"
      RUNNING_COUNT="-"
    }
